...
async function sendRBImageTemplate(numbers, idSensor, urlImageRB) {
    //"524442478772" --> Devie
    //524442478574 --> Ruben

    let models = [];
    console.log("Enviando mensajes DESDE: ", urlImageRB);

    //Construir los modelos de los templates para cada usuario
    for (const number of numbers) {
        let model = whatsAppModel.TemplateRB(number, idSensor, urlImageRB);

        models.push(model); // Agregar el modelo construido a la lista
    }

    console.log("Enviando mensajes de TEMPLATE Radio Base");
    ...
    try {
        ...
        // Manejar errores generales
        console.error("TUVIMOS UN ERROR EN LAS PLANTILLAS de Radio Base: ", error, error.stack);
    }
}
...
async function sendClientTemplate(numbers, textBuilt, ip) {
    //"524442478772" --> Devie
    //524442478574 --> Ruben
    ...
    //Construir los modelos de los templates para cada usuario
    for (const number of numbers) {
        let model = whatsAppModel.TemplateClientes(number, textBuilt, ip);

        models.push(model); // Agregar el modelo construido a la lista
    }

    console.log("Enviando mensajes de TEMPLATE Clientes");
